...
export function AdminReviewListPage() {
  ...
  const load = () => {
    setLoading(true)
    setError('')
    api.moderationDrafts()
      .then(setDrafts)
      .catch((reason) => setError(reason instanceof Error ? reason.message : '검수 목록을 불러오지 못했습니다.'))
  ...
  const filtered = useMemo(() => drafts.filter((draft) => {
    const placeMatches = place === '전체 장소' || draft.placeName === place
    const dateMatches = !date || draft.createdAt.startsWith(date)
    return placeMatches && dateMatches
  }), [date, drafts, place])
    ...
    <AdminShell active="review" searchPlaceholder="제보 번호 또는 장소 검색...">
      <div className="admin-page admin-review-list-page">
        <header className="admin-review-list-heading">
          <div><h1>검수 대기 목록</h1><p>AI가 구조화한 실제 제보 데이터를 검토하고 승인하세요.</p></div>
          <span className="admin-count-pill">총 <b>{drafts.length}</b>건 대기 중</span>
        </header>
        ...
        <section className="admin-review-table-card">
          ...
          {filtered.map((draft) => (
            <div className="admin-review-record" key={draft.draftId}>
              <div className="admin-review-table__row">
                <span>DRAFT-{draft.draftId}</span>
                <span className="admin-review-place"><img src={pinIcon} alt="" />{draft.placeName}</span>
                <span>{formatDate(draft.createdAt)}</span>
                <span className="align-right">REP-{draft.reportId}</span>
                <span className="admin-review-summary">{draft.summary}</span>
                <span className="align-center"><em className="admin-review-state">검수 대기</em></span>
                <span className="align-center"><button className="admin-table-action" type="button" onClick={() => navigate(`/admin/reviews/${draft.draftId}`)}>검수하기</button></span>
              </div>
            </div>
          ))}
