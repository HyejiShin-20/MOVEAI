...
=======
  ...
  useEffect(() => {
    ...
    api.places()
      .then((response) => {
        ...
        setPlaces(response)
        if (!selectedPlace) {
          setSelectedPlace(response.find((place) => place.placeCode === 'PLACE_B')?.id ?? response[0]?.id ?? null)
        }
      })
  ...
  const filteredPlaces = places.filter((place) =>
    !query.trim() || place.name.toLocaleLowerCase().includes(query.trim().toLocaleLowerCase()),
  )
  ...
  const complete = () => {
    ...
    reportDraftStore.patch({
      selectedPlace: {
        ...
        address: selectedPlaceData.description ?? selectedPlaceData.placeType,
      },
      ...
      <main className="place-main">
        <section className="place-search">
          ...
          <label>
            ...
            <input type="search" placeholder="등록 장소 검색" value={query} onChange={(event) => setQuery(event.target.value)} />
          </label>
...
=======
            <button className="place-complete" type="button" disabled={!selectedPlaceData} onClick={complete}>
>>>>>>> Stashed changes
